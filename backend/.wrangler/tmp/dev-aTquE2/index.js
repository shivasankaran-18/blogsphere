...
// .wrangler/tmp/bundle-ixRsDb/checked-fetch.js
...
// .wrangler/tmp/bundle-ixRsDb/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-ixRsDb/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
app.post("/api/v1/user/signup", async (c) => {
  ...
  const token = await sign2({ email: body.email }, Secret_default);
  console.log(token);
  return c.json({ token });
});
app.post("/api/v1/user/signin", async (c) => {
  ...
  const token = sign2({ email: body.email }, Secret_default);
  return c.json({
    token
  });
});
